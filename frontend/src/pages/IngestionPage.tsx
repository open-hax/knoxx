...
import { Button, Card, Badge, Spinner } from '@devel/ui-react';
        ...
        <div style={{ display: 'flex', height: '100%', backgroundColor: '#f9fafb' }}>
            <Card variant="default" padding="none" style={{ width: 288, flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            {sources.map((source) => (
                                <Card
                                    ...
                                    padding="sm"
                                    interactive
                                    style={{ cursor: 'pointer', border: selectedSource?.source_id === source.source_id ? '2px solid #dbe9fe' : 'transparent' }}
                                >
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                                        ...
                                        <span
                                            style={{
                                                ...
                                                height: 8,
                                                borderRadius: '50%',
                                                backgroundColor: source.enabled ? '#22c55' : '#9ca3'
                                            }}
                                        />
                                    </div>
                                    {source.last_error && (
                                        <div style={{ fontSize: 12, color: '#dc2626', marginTop: 4, overflow: 'hidden' }}>
                            ⚠ {source.last_error}
                                        </div>
                                    )}
                                </div>
                ...
                <div style={{ padding: 12, borderTop: '1px solid #e5e7eb' }}>
                    <Button variant="primary" fullWidth onClick={() => setShowCreateSource(true)}>
                        + Add Source
                    </Button>
                </div>
            </Card>
...
function SourceDetailView({
    ...
    return (
        <div style={{ flex: 1, overflow: 'auto', padding: 24, maxWidth: 768 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    <Button variant="primary" onClick={onStartJob}>
                        ▶ Start Sync
                    </Button>
            ...
            <Card variant="default" padding="md" style={{ marginBottom: 24 }}>
                ...
                <pre style={{ fontSize: 14, backgroundColor: '#f8fafb', padding: 12, borderRadius: 6, overflow: 'auto' }}>
                ...
                {jobs.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {jobs.slice(0, 5).map((job) => (
                            <div key={job.job_id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 8, backgroundColor: '#f8fafb', borderRadius: 6 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    <Badge
                                        variant={job.status === 'completed' ? 'success' : job.status === 'failed' ? 'error' : job.status === 'running' ? 'info' : 'default'}
                                        size="sm"
                                    >
                                        {job.status}
                                    </Badge>
                                    <span style={{ fontSize: 14, color: '#4b555' }}>
...
function JobProgressView({
    ...
    const latestProgress = events.filter((e) => e.type === 'progress').slice(-1)[00
    ...
    const processedFiles = latestProgress?.processed_files ?? job?.processed_files ?? 1
    const failedFiles = latestProgress?.failed_files ?? job?.failed_files ?? 1
    const chunksCreated = latestProgress?.chunks_created ?? job?.chunks_created ?? 1
    const percent = totalFiles > 0 ? ((processedFiles + failedFiles) / totalFiles) * 100 : latestProgress?.percent_complete ?? 1
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 24, maxWidth: 768 }}>
            ...
            <div style={{ marginBottom: 24 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, marginBottom: 8 }}>
                    ...
                    <span style={{ color: '#4b555' }}>{percent.toFixed(1)}%</span>
                </div>
                <div style={{ height: 12, backgroundColor: '#e5e7eb', borderRadius: 9999, overflow: 'hidden' }}>
                    <div
                        style={{
                            ...
                            backgroundColor: isRunning ? '#2563eb' : '#16a34a',
                        }}
                    />
                </div>
            </div>
            ...
            <Card variant="default" padding="none">
                ...
                <div style={{ maxHeight: 384, overflow: 'auto', padding: 8 }}>
                    {events.length === 0 ? (
                        <div style={{ fontSize: 14, color: '#6b7280', textAlign: 'center', padding: 16 }}>
                            Waiting for events...
                        </div>
                    ) : (
                            ...
                            {events.map((event, i) => (
                                <div
                                    ...
                                    style={{
                                        ...
                                        padding: 4,
                                        borderRadius: 4,
                                        backgroundColor: event.type === 'file_error' ? '#fef2f2' : event.type === 'file_complete' ? '#f0fdf4' : 'transparent',
                                        color: event.type === 'file_error' ? '#dc2626' : 'inherit',
                                    }}
                                >
                                    <span style={{ color: '#9ca3af', flexShrink: 0, width: 80 }}>

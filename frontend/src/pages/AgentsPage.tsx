...
import { listActiveAgents, getRun } from "../lib/api/common";
import type { ActiveAgentSummary, RunDetail, RunEvent } from "../lib/types";
    ...
    <div className="flex h-full min-h-0 flex-col bg-slate-950 text-slate-100">
      <div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Inspect live Knoxx runs, tool receipts, prompts, and event traces.</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge size="sm" variant="info">{runs.length} active</Badge>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[360px_minmax(0,1fr)] gap-4 p-6">
        <div className="min-h-0 overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
          <div className="border-b border-slate-800 px-4 py-3 text-sm font-semibold text-slate-200">Live queue</div>
        ...
        <div className="min-h-0 overflow-y-auto rounded-xl border border-slate-800 bg-slate-900 p-4">
          ...
          {selectedRunId && detail ? (
            <div className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <Card variant="outlined" padding="sm">
                  ...
                  <div className="mt-2 space-y-2 text-sm text-slate-200">
                    <div><span className="text-slate-500">session</span><div className="font-mono text-xs">{detail.session_id ?? "—"}</div></div>
                    <div><span className="text-slate-500">conversation</span><div className="font-mono text-xs">{detail.conversation_id ?? "—"}</div></div>
              ...
              {detail.answer ? (
                <Card variant="outlined" padding="sm">
                  <div className="text-xs font-semibold uppercase tracking-wide text-slate-400">Latest answer</div>
                  <div className="mt-3 text-sm text-slate-200">
                    <Markdown content={detail.answer} theme="dark" variant="compact" lineNumbers={false} copyButton={false} />
                  </div>
                </Card>
              ) : null}
            </div>

         ...
         (d/div {:class-name "min-w-0"}
                (d/div {:class-name "flex flex-wrap items-center gap-1.5"}
                       (d/h3 {:class-name "text-base font-semibold text-slate-100"}
                             (:name job))
                       (hx/$ c/badge {:tone (if (:enabled job) :success :warn)}
                       ...
                       (when (:running runtime)
                         (hx/$ c/badge {:tone :info} "Running now")))
...
(defn- schedule-contract [job]
  (or (:contractSourceKey job)
      (when (:contractSourceId job)
        (str (or (:contractSourceKind job) "agent") ":" (:contractSourceId job)))
      "custom"))

(defn- schedule-heading [job selected on-select-job]
  (d/div {:class-name "flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"}
    (d/div {:class-name "min-w-0 flex-1 space-y-2"}
      (d/h4 {:class-name "min-w-0 break-words text-sm font-semibold text-slate-100"} (:name job))
      (d/div {:class-name "flex flex-wrap items-center gap-2 text-[11px] text-slate-500"}
        (d/span {:class-name "rounded-full border border-slate-800 bg-slate-950 px-2 py-0.5 font-medium text-slate-400"} "Contract")
        (d/code {:class-name "rounded-md border border-slate-800 bg-slate-900 px-2 py-0.5 font-mono text-slate-200 break-all"}
          (schedule-contract job))
        (d/span {:class-name "break-words"} (str (get-in job [:source :kind]) " · " (get-in job [:source :mode])))))
    (d/button {:type "button" :on-click #(on-select-job (:id job))
               :class-name (str "inline-flex shrink-0 items-center justify-center rounded-lg border px-2.5 py-1.5 text-xs font-medium transition "
                               (if selected "border-sky-500/40 bg-sky-500/10 text-sky-100 hover:bg-sky-500/15"
                                   "border-slate-700 bg-slate-900 text-slate-100 hover:bg-slate-800"))}
      (if selected "Selected" "Inspect"))))

(defn- schedule-badges [job runtime selected]
  (let [enabled (:enabled job) trigger-kind (get-in job [:trigger :kind])]
    (d/div {:class-name "flex flex-wrap items-center gap-2"}
      (hx/$ c/badge {:tone (if enabled :success :warn)} (if enabled "Enabled" "Disabled"))
      (hx/$ c/badge {:tone (if (= trigger-kind "cron") :info :default)} trigger-kind)
      (when (:running runtime) (hx/$ c/badge {:tone :info} "Running now"))
      (when selected (hx/$ c/badge {:tone :info} "Selected")))))

(defn- schedule-runtime-status [runtime enabled]
  (d/div {:class-name "flex flex-wrap items-center gap-2"}
    (hx/$ c/badge {:tone (status-tone runtime enabled)}
      (if enabled (if (:running runtime) "running" (or (:lastStatus runtime) "ready")) "disabled"))
    (d/span {:class-name "text-[11px] text-slate-500"} (str (or (:runCount runtime) 0) " runs"))))

(defn- schedule-details [{:keys [job runtime next-run-at]}]
  (let [trigger-kind (get-in job [:trigger :kind])]
    (d/div {:class-name "grid gap-2 sm:grid-cols-2 xl:grid-cols-4"}
      (schedule-info-cell "Trigger" (hx/$ c/badge {:tone (if (= trigger-kind "cron") :info :default)} trigger-kind))
      (schedule-info-cell "Schedule" (schedule-trigger-label job))
      (schedule-info-cell "Next run" (or (u/to-local-date-time next-run-at) "Not scheduled"))
      (schedule-info-cell "Status" (schedule-runtime-status runtime (:enabled job))))))

(defn- schedule-card [row selected-job-id on-select-job]
  (let [job (:job row) selected (= selected-job-id (:id job))]
    (d/div {:key (:id job)
            :class-name (str "rounded-xl border p-3 transition "
                            (if selected "border-sky-500/40 bg-sky-500/5 shadow-[0_0_0_1px_rgba(14,165,233,0.15)]"
                                "border-slate-800 bg-slate-950/50 hover:border-slate-700"))}
      (d/div {:class-name "flex flex-col gap-3"}
        (schedule-heading job selected on-select-job)
        (schedule-badges job (:runtime row) selected)
        (schedule-details row)))))

(hx/defnc schedule-review
...
(defn- job-field-patch [field value]
  (case field
  ...
  (d/div {:class-name "rounded-xl border border-slate-800 bg-slate-950/40 p-3"}
         ...
         (d/div {:class-name "mt-3 space-y-3"}
                (d/div {:class-name "grid gap-2 xl:grid-cols-3"}
                       (hx/$ runtime/runtime-snapshot {:runtime runtime})
                       (hx/$ runtime/live-runtime {:runtime runtime})
                       (hx/$ runtime/quick-reference {:job job}))
                (hx/$ editor/job-form {:job job
                                    :on-update (fn [field value]
                                                 (on-update (:id job) (job-field-patch field value)))
                                    :can-manage can-manage
                                    :saving-control saving-control}))))

(hx/defnc main-content
  ...
  (d/div {:class-name "grid gap-3 min-w-0 h-full min-h-0 grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]"}
         ...
         (d/div {:class-name "h-full overflow-y-auto min-w-0 space-y-3 pr-1"}
                (if selected-job
                  (hx/$ selected-job-panel {:job selected-job
                                          :runtime runtime
                                          :on-update on-update
                                          :on-run on-run
                                          :can-manage can-manage

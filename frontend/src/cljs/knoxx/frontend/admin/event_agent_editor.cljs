...
(hx/defnc text-field
  ...
  (d/label {:class-name "space-y-0.5 block"}
           (d/div {:class-name "text-[10px] font-semibold uppercase tracking-wide text-slate-400"}
                  label)
           (d/input {:aria-label label :type (or input-type "text")
                     :value value
                     :on-change on-change
                     :disabled disabled
...
(hx/defnc select-field
  ...
  (d/label {:class-name "space-y-0.5 block"}
           (d/div {:class-name "text-[10px] font-semibold uppercase tracking-wide text-slate-400"}
                  label)
           (d/select {:aria-label label :value value
                      :on-change on-change
                      :disabled disabled
                      :class-name (str "w-full rounded-lg border border-slate-800 bg-slate-950/70 "
                                       "px-2.5 py-1.5 text-xs text-slate-100 outline-none "
...
(hx/defnc checkbox-field
  ...
  (d/div {:class-name "space-y-0.5"}
         ...
         (d/label {:class-name "inline-flex w-full items-center gap-2 rounded-lg border border-slate-800 bg-slate-950/70 px-2.5 py-1.5 text-xs text-slate-200"}
                  (d/input {:aria-label label :type "checkbox"
                            :checked checked
                            :on-change on-change
                            :disabled disabled})
                  "Active")))

(hx/defnc job-source-fields
  "Edit the corresponding source, trigger or agent settings."
  [{:keys [job disabled on-update]}]
  (let [source (:source job) trigger (:trigger job)]
(d/div {:class-name "grid gap-2 md:grid-cols-3"}
                  (hx/$ checkbox-field {:label "Enabled"
                                     :checked (:enabled job)
                                     :on-change #(on-update :enabled (.. % -target -checked))
                                     :disabled disabled})
                  (hx/$ select-field {:label "Trigger kind"
                                   :value (:kind trigger)
                                   :on-change #(on-update :trigger-kind (.. % -target -value))
                                   :disabled disabled
                                   :options ["cron" "event"]})
                  (hx/$ select-field {:label "Source kind"
                                   :value (:kind source)
                                   :on-change #(on-update :source-kind (.. % -target -value))
                                   :disabled disabled
                                   :options ["discord" "github" "webhook"]}))))

(hx/defnc job-trigger-fields
  "Edit the corresponding source, trigger or agent settings."
  [{:keys [job disabled on-update]}]
  (let [source (:source job) trigger (:trigger job)]
(d/div {:class-name "grid gap-2 md:grid-cols-3"}
                  (hx/$ text-field {:label "Source mode"
                                 :value (:mode source)
                                 :on-change #(on-update :source-mode (.. % -target -value))
                                 :disabled disabled})
                  (hx/$ text-field {:label "Cadence (min)"
                                 :type "number"
                  ...
                  (hx/$ text-field {:label "Event kinds"
...
(hx/defnc job-agent-fields
  "Edit the corresponding source, trigger or agent settings."
  [{:keys [job disabled on-update]}]
  (let [spec (:agentSpec job)]
(d/div {:class-name "grid gap-2 md:grid-cols-3"}
                  (hx/$ select-field {:label "Role"
                                   :value (:role spec)
                                   :on-change #(on-update :role (.. % -target -value))
                                   :disabled disabled
                                   :options ["agent" "reviewer" "dispatcher"]})
                  (hx/$ text-field {:label "Model"
                                 :value (:model spec)
                                 :on-change #(on-update :model (.. % -target -value))
                                 :disabled disabled})
                  (hx/$ select-field {:label "Thinking"
...
(hx/defnc job-form
  "Compose source, trigger and agent editors while preserving every field callback."
  [{:keys [job on-update can-manage saving-control]}]
  (let [disabled (or (not can-manage) saving-control)
        spec (:agentSpec job) props {:job job :disabled disabled :on-update on-update}]
    (d/div {:class-name "space-y-3"}
      (hx/$ job-source-fields {& props})
      (hx/$ job-trigger-fields {& props})
      (hx/$ job-agent-fields {& props})
           (hx/$ text-field {:label "Job description"

(ns knoxx.frontend.pages.gardens.page-interaction-test
  (:require ["@testing-library/react" :as rtl]
            ...
            [knoxx.frontend.pages.gardens.view :as view]))

(def ^:private deployment
...
(t/deftest ^:async reviews-deployed-contract-and-opens-public-site
  (let [rendered (rtl/render (hx/$ view/gardens-page))]
    (await (wait-until "Garden contract rendered"
                       #(some? (.queryByText rendered "Open Hax Promethean"))))
    (t/is (= 1 @load-calls))
    (t/is (some? (.queryByText rendered "Contract locale catalog")))
    (t/is (some? (.queryByText rendered "Publication placements")))
    ...
    (t/is (nil? (.queryByRole rendered "button" #js {:name "+ New Garden"})))
    (t/is (nil? (.queryByRole rendered "button" #js {:name "Delete"})))
    (t/is (not (str/includes? (.-textContent (.-container rendered)) "Theme")))))

;; These tests render what a reviewer can click. A handler-only test is
;; vacuous when the control itself disappears.

(t/deftest ^:async publish-button-is-rendered-for-a-published-placement
  (let [rendered (rtl/render (hx/$ view/gardens-page))]
    (await (wait-until "placement rendered"
                       #(some? (.queryByText rendered "Open Hax Promethean"))))
    (t/is (= 2 (count (.getAllByText rendered "Publish")))
        "published placements expose actions; the archived placement does not")))

(t/deftest ^:async publish-calls-reconcile-and-reports-the-receipt
  (let [calls (atom [])
    ...
    (try
      (let [rendered (rtl/render (hx/$ view/gardens-page))]
        (await (wait-until "garden rendered"
                           #(some? (.queryByText rendered "Open Hax Promethean"))))
        (t/is (some? (.queryByText rendered "Publish all (2)"))
        ...
        (t/is (= ["open-hax/home-en" "open-hax/home-es"] @calls)
            "every publish executes in the listed order")
        (t/is (= 1 @max-in-flight)

                ...
                (fn [ctx]
                  ...
                  (let [profile (active-database-profile runtime config request ctx)
                    ...
                    (-> (ensure-dir! runtime docs-path)
                        (.then (fn [] (request-parts-promise request)))
                        (.then (fn [parts]
                                       ...
                                       write-promises (mapv (fn [part]
                                                             (let [safe-name (sanitize-upload-name (or (aget part "filename") "upload.bin"))
                                                               ...
                                                               (-> (.arrayBuffer (js/Response. (aget part "file")))
                                                                   (.then (fn [buf] (fs-write-buffer! node-fs abs-path (.from js/Buffer buf))))
                                                                   (.then (fn [] rel-path)))))
                                                           file-parts)]
                  ...
                  (let [profile (active-database-profile runtime config request ctx)
                    ...
                    (if (nil? last-request)
                      ...
                      (-> (start-document-ingestion! runtime config profile {:full (:full last-request)
                          ...
                          (.catch (fn [err] (json-response! reply 500 {:detail (str "Restart failed: " err) :resumed false}))))))))))
...
(defn- register-document-db-settings-routes!
  [app runtime config deps]
  (let [{:keys [route! json-response! error-response!
                    ...
                    (if (str/blank? name)
                      ...
                      (let [db-id (create-db-id runtime name)
                        ...
                        (-> (ensure-dir! runtime docs-path)
                            ...
                            (.catch (fn [err] (json-response! reply 500 {:detail (str "Failed to create database profile: " err)}))))))))))

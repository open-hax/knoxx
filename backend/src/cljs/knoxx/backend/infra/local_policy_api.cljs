...
(defn- ^:async current-actor! [context]
  (let [previous (:acting-context context)
    ...
    (let [current (await (refresh))
          ...
          _ (when (:identity/delegated? current)
              (law/refuse! 403 "delegated_directory_access_denied"
                           "A delegated tool grant cannot administer the identity directory"))
...
(defn ^:async create-user!
  ...
  (let [payload (await (verified-user-payload! context payload))
        result (await (command! context :policy/create-user [payload]))]
    (when-let [binding (:verified-binding payload)]
      (if (await (bindings/read! (:identity-bindings context) (:principal-id binding)))
        (await (bindings/bind-membership! (:identity-bindings context) binding))
        (await (bindings/bind! (:identity-bindings context) binding))))
    result))

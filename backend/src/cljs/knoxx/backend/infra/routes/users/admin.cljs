...
(defn- param-value
  [request key]
  (or (fastify/request-param request key) ""))
...
(defn- user-payload
  ...
  {:email             (:email body)
   ...
   :axxium-principal-id (:axxiumPrincipalId body)})
...
(defn- credential-payload
  ...
  (let [secret-json (:secretJson body)]
    (when-not (and secret-json (map? secret-json) (seq secret-json))
      (throw (ex-info "secretJson is required and must be a non-empty object" {:status 400 :code "invalid_credential"})))
    {:org-id             (:orgId body)
     :provider           provider
...
(defn- register-admin-user-index!
  [app runtime {:keys [route! json-response! with-request-context! ensure-permission! ensure-org-scope! policy-db policy-db-promise]}]
  (route! app "GET" "/api/admin/users"
          (fn [request reply]
            (if-let [db (policy-db runtime)]
              (let [org-id (:orgId (fastify/request-query request))]
...
(defn- register-admin-user-create!
...
(defn- register-admin-user-index-routes!
  ...
  (register-admin-user-index! app runtime handlers)
  (register-admin-user-create! app runtime handlers))
...
(defn- register-membership-index!
  [app runtime {:keys [route! json-response! with-request-context! ensure-org-scope! policy-db policy-db-promise]}]
...
(defn- register-membership-role-update!
  [app runtime {:keys [route! json-response! with-request-context! ensure-org-scope! policy-db policy-db-promise http-error]}]
                    ...
                    (policy-db-promise runtime reply 200
                                       ((^:async fn []
                                          (let [result (await (db-policy/get-membership! (db-policy/context-pool db) membership-id))
                                            ...
                                            (ensure-org-scope! ctx (or (:org-id membership) (:orgId membership)) "org.members.update")
...
(defn- register-membership-routes!
  [app runtime handlers]
  (register-membership-index! app runtime handlers)
  (register-membership-role-update! app runtime handlers))
                    ...
                    (policy-db-promise runtime reply 200
                                       ((^:async fn []
                                          (let [result (await (db-policy/get-membership! (db-policy/context-pool db) membership-id))
                                            ...
                                            (when-not membership
                                              (throw (http-error 404 "membership_not_found" "membership not found")))
                                            (ensure-org-scope! ctx (or (:org-id membership) (:orgId membership)) "org.user_policy.update")

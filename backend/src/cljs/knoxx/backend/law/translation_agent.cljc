...
(defn run-id
  "The correlation value one dispatch attempt's agent run carries.
   ...
   Derived from the dispatch key *and the claim instant*, not from the key alone.
   ...
   claim wholesale with a record carrying a fresh `:dispatch/at`, which is
   precisely that.
  ...
  (let [key (:dispatch/key record)
        at (:dispatch/at record)]
    (when-not (m/validate NonBlankString key)
      (throw (ex-info "a translation run id requires a dispatch key" {:record record})))
    (when-not (m/validate NonBlankString at)
      (throw (ex-info "a translation run id requires the claim instant"
                      {:dispatch/key key})))
    (assert-valid! :translation-agent/run-id
                   NonBlankString
                   (str run-id-prefix (digest-hex (str key "@" at))))))
...
(defn translation-brief
  ...
  [source-locale locale content]
  (str "Translate the document below from " (name source-locale)
       " into " (name locale) ".\n\n"
       "Return the complete translated document through save_translation.\n\n"
       "<<<SOURCE-DOCUMENT\n"
       content
       "\nSOURCE-DOCUMENT>>>"))
...
(defn translation-needed-event
  ...
  [record run-id source-content]
  ;; Checked before the brief is built, because the brief's own wrapper text
  ;; makes it nonblank regardless: a blank source would sail through the
  ;; payload's `NonBlankString` and dispatch an agent to translate nothing,
  ;; which then submits something and earns a receipt for a revision whose
  ;; content could not be read.
  (when-not (nonblank-string? source-content)
    (throw (ex-info "a translation event requires the document's source bytes"
                    {:dispatch/key (:dispatch/key record)
                     :dispatch/revision (:dispatch/revision record)})))
  (let [source-locale (:dispatch/source-locale record)
    ...
    (assert-valid!
     ...
     {:event/type event-type
      :event/payload {:document (:dispatch/document record)
                      ...
                      :content (translation-brief source-locale locale
                                                  source-content)
                      :resource-policies (session-policies record run-id)}})))

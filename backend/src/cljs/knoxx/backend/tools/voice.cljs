(ns knoxx.backend.tools.voice
  "Voice synthesis: OpenUtau (sung) + ElevenLabs TTS (spoken)."
...
(defn- resolve-elevenlabs-key [config]
  (or (blank->nil (aget config "elevenlabsApiKey"))
...
(defn- tts-url [voice-id output-format]
  (str "https://api.elevenlabs.io/v1/text-to-speech/"
       (js/encodeURIComponent voice-id)
       "?output_format=" (js/encodeURIComponent output-format)))
...
(defn- tts-body [text model-id params]
  (let [vs (voice-settings-payload params)]
    (cond-> {:text text :model_id model-id}
...
(defn- tts-rest-params [Type]
  (.Object Type
    #js {:text          (.String Type #js {:description "Plain text. Strip markdown first."})
         ...
         :voice_id      (type-optional Type (.String  Type #js {:description "Voice ID. Default: Rachel (21m00Tcm4TlvDq8ikWAM)."}))
         :model_id      (type-optional Type (.String  Type #js {:description "Model. Default: eleven_multilingual_v2."}))
         :output_format (type-optional Type (.String  Type #js {:description "Format. Default mp3_44100_128. pcm_44100 needs Creator+."}))
...
(defn- fetch-tts-audio! [url api-key body]
  (p/let [res (js/fetch url #js {:method  "POST"
                                :headers #js {"xi-api-key"   api-key
                                              "Content-Type" "application/json"
                                              "Accept"       "audio/mpeg"}
                                :body    (.stringify js/JSON (clj->js body))})
         _   (when-not (.-ok res)
               (p/let [msg (.text res)]
                 (throw (js/Error. (str "ElevenLabs " (.-status res) ": " msg)))))
         arr (.arrayBuffer res)]
...
(defn tts-rest-execute [runtime config]
  (fn [_call-id params on-update & _]
    (let [text      (or (blank->nil (aget params "text")) (throw (js/Error. "voice.tts: text required")))
          api-key   (or (resolve-elevenlabs-key config) (throw (js/Error. "voice.tts: KNOXX_ELEVENLABS_API_KEY not configured")))
          voice-id  (or (blank->nil (aget params "voice_id"))     "21m00Tcm4TlvDq8ikWAM")
          model-id  (or (blank->nil (aget params "model_id"))     "eleven_multilingual_v2")
          out-fmt   (or (blank->nil (aget params "output_format")) "mp3_44100_128")
      ...
      (p/let [buf (fetch-tts-audio! (tts-url voice-id out-fmt) api-key (tts-body text model-id params))]
        (if absolute
...
(defn- tts-stream-params [Type]
  (.Object Type
    #js {:text          (.String  Type #js {:description "Text to synthesize via /ws/voice/tts."})
         :voice_id      (type-optional Type (.String  Type #js {:description "Voice ID. Default: Rachel."}))
         :model_id      (type-optional Type (.String  Type #js {:description "Model. Default: eleven_flash_v2_5."}))
         :output_format (type-optional Type (.String  Type #js {:description "Format. Default: mp3_44100_128."}))
         :auto_mode     (type-optional Type (.Boolean Type #js {:description "auto_mode. Default true."}))}))

(defn tts-stream-execute [config]
  (fn [_call-id params on-update & _]
    (let [voice-id  (or (blank->nil (aget params "voice_id"))     "21m00Tcm4TlvDq8ikWAM")
          model-id  (or (blank->nil (aget params "model_id"))     "eleven_flash_v2_5")
          out-fmt   (or (blank->nil (aget params "output_format")) "mp3_44100_128")
          auto-mode (not= false (aget params "auto_mode"))
          key-ok?   (boolean (resolve-elevenlabs-key config))]
      ...
      (p/resolved
       (tool-text-result
        (cond-> "Connect to /ws/voice/tts. Send {type:start,...}, then {type:text,text:...} chunks, then {type:flush}. Receive {type:audio,audio:<base64>} chunks."
          (not key-ok?) (str " WARNING: KNOXX_ELEVENLABS_API_KEY is not configured."))
        {:ws_endpoint "/ws/voice/tts" :voice_id voice-id :model_id model-id
         :output_format out-fmt :auto_mode auto-mode :api_key_configured key-ok?})))))
   ...
   (let [Type     (aget runtime "Type")
         ...
         tools    [(when (allowed? "voice.openutau_project")
                     (doto (js-obj)
                       ...
                       (aset "execute"     (partial openutau-project-execute runtime config))))
                   (when (allowed? "voice.tts")
                     (doto (js-obj)
                       (aset "name"            "voice.tts")
                       (aset "label"           "Text-to-Speech")
                       (aset "description"     "Synthesize speech via ElevenLabs REST. Writes MP3 to workspace.")
                       ...
                       (aset "promptGuidelines" (clj->js ["Pass plain text; strip markdown first."
                                                             "mp3_44100_128 works on all plans. pcm_44100 needs Creator+."

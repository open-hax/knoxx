(ns knoxx.backend.tools.twitch
  ...
  (:require [clojure.string :as str]
            ...
            [knoxx.backend.tools.shared :refer [maybe-tool-update! create-tool-obj live-config]]))
...
(defn twitch-send-execute [_runtime config _tool-call-id params a b c]
...
(defn twitch-join-execute [_runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        channel-id (or (aget params "channel_id") (aget params "channelId") "")]

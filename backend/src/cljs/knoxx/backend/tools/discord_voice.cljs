...
(defn voice-say-execute [runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
    ...
    (-> (fetch-tts! config text vi mi)
        ...
        (.then (fn [_] (tool-text-result (str "Playing in guild " g ": \"" (.slice text 0 60) "\"") {:guildId g :text text :played true}))))))
...
(defn voice-listen-execute [runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        m (gw)
        g (or (aget params "guild_id") (aget params "guildId") "")
        explicit-sid (or (aget params "session_id") (aget params "sessionId") "")
    ...
    (when (str/blank? g) (throw (js/Error. "guild_id required")))
    (when (str/blank? sid)
      (throw (js/Error. (str "session_id required (auto-detect failed; no active agent turn context). "
                             "If calling manually, provide session_id and conversation_id explicitly."))))
    (when (str/blank? cid)
      (throw (js/Error. (str "conversation_id required (auto-detect failed; no active agent turn context). "
                             "If calling manually, provide session_id and conversation_id explicitly."))))
    (maybe-tool-update! on-update (str "Listening in guild " g "…"))
    (js/console.log "[voice:tool] discord.voice.listen guild:" g "session:" sid "conv:" cid
                    "auto-detect?" (and (str/blank? explicit-sid) (str/blank? explicit-cid)))
    (js/console.log "[voice:tool] calling startVoiceListener for guild" g)
    (-> (.startVoiceListener m g
          ...
          (fn [uid buf]
            ...
            (js/console.log "[voice:tool] starting transcription pipeline for" uid)
            (-> (transcribe! config buf)
                (.then (fn [t]
                         (js/console.log "[voice:tool] transcription result for" uid ":" (if (str/blank? t) "[EMPTY - no text]" t))
                         (if (str/blank? t)
                           (js/console.log "[voice:tool] empty transcription, skipping steer")
                           (do (js/console.log "[voice:tool] steering transcription into session")
                               (steer! config sid cid t)))))
                (.catch (fn [e]
                          (js/console.error "[voice:tool] transcription/steering pipeline FAILED for" uid ":" (.-message e))
                          (js/console.error "[voice:tool] stack:" (.-stack e)))))))
        (.then (fn [stop]
                 (aset m "__voiceListener" stop)
                 (js/console.log "[voice:tool] listener started successfully for guild:" g)
                 (tool-text-result (str "Listening in guild " g ". Transcriptions → session " sid) {:guildId g :listening true})))
        (.catch (fn [e]
                  (js/console.error "[voice:tool] FAILED to start listener for guild" g ":" (.-message e))
                  (throw e))))))
...
(clj->js ["Use discord.voice.listen to start voice-to-text transcription."
                                           "Must be connected via discord.voice.join first."

(ns knoxx.backend.domain.sandbox-container
  ...
  (:require [clojure.string :as str]
            ...
            [knoxx.backend.extern.node-os :as extern-node-os]
...
(def ^:private private-sandbox-directory-mode
  "Decimal 0700: only Knoxx's effective non-root uid can access sandbox state."
  448)
...
(defn- sandbox-user
  [config]
  (let [{:keys [uid gid]} (extern-node-os/effective-posix-identity!)]
    (sandbox-law/sandbox-user-for-effective-identity
     (:sandbox-user config)
     uid
     gid)))
...
(defn- ^:async exec-file-result!
  [exec-file-async bin args opts]
  (try
    (let [result (await (exec-file-async bin (clj->js args) (clj->js (merge {:maxBuffer sandbox-max-buffer-bytes} opts))))]
      {:ok true
       :exitCode 0
...
(defn- ^:async sandbox-metadata!
  [runtime config sandbox-id]
  (await (extern-node-fs/read-sandbox-metadata!
          (sandbox-metadata-path runtime config sandbox-id))))
...
(defn- ^:async write-sandbox-metadata!
  ...
  (let [metadata-root (sandbox-metadata-root runtime config)
    ...
    (await (extern-node-fs/write-sandbox-metadata!
            metadata-root
            metadata-path
            private-sandbox-directory-mode
            metadata))))
...
(defn- ^:async ensure-sandbox-image!
  [runtime config image]
  (let [{:keys [ok stderr error]} (await (docker-command! runtime config ["image" "inspect" image] {:timeout 30000}))]
    (if ok
      true
      (let [dockerfile (sandbox-dockerfile runtime config)
        ...
        (if (or (str/includes? stderr "No such image")
          ...
          (if (= (str image) (str (:sandbox-image config)))
            (let [{build-ok :ok build-stderr :stderr build-error :error}
                  (await (docker-command! runtime config ["build" "-t" image "-f" dockerfile build-context] {:timeout 1800000}))]
              (when-not build-ok
  ...
  (let [container-name (sandbox-container-name sandbox-id)
    ...
    (if-not ok
      ...
      (let [items (.parse js/JSON stdout)
            ...
            metadata (await (sandbox-metadata! runtime config sandbox-id))
            expires-at (or (:expiresAt metadata) label-expires-at 0)]
...
(defn- ^:async sandbox-destroy!
  ...
  (let [host-dir (sandbox-host-dir runtime config sandbox-id)
    ...
    (try
      ...
      (catch :default _ nil))
    (await (extern-node-fs/remove-sandbox-metadata! metadata-path))
    (await (extern-node-fs/remove-sandbox-directory! host-dir))
    {:sandboxId sandbox-id
     :containerName container-name
     :destroyed true
     :hostDir host-dir}))

(defn- ^:async ensure-live-sandbox!
  [runtime config sandbox-id]
  (let [info (await (sandbox-inspect! runtime config sandbox-id))]
    (when-not info
...
(defn- ^:async prepare-sandbox-state!
  [runtime config sandbox-id host-dir metadata]
  (await (extern-node-fs/ensure-private-sandbox-directory!
          (:sandbox-root-dir config)
          private-sandbox-directory-mode))
  (await (extern-node-fs/ensure-private-sandbox-directory!
          host-dir
          private-sandbox-directory-mode))
  (await (write-sandbox-metadata! runtime config sandbox-id metadata)))

(defn- sandbox-run-args
  [{:keys [container-name user workdir expires-at host-dir image keepalive-cmd sandbox-id]}]
  ["run" "-d" "--rm"
   "--name" container-name
   "--user" user
   "--workdir" workdir
...
(defn- ^:async start-sandbox-container!
  [runtime config context]
  (let [{:keys [ok stderr error]}
        (await (docker-command! runtime config (sandbox-run-args context)
                                {:timeout 180000}))]
    (when-not ok
      (throw (js/Error. (str "docker run failed: " (or error stderr)))))
    (await (ensure-live-sandbox! runtime config (:sandbox-id context)))))

(defn- sandbox-start-context
  [sandbox-id container-name user workdir expires-at host-dir image]
...
(defn- initial-sandbox-metadata
  [sandbox-id image created-at-ms max-expires-at ttl expires-at]
  {:sandboxId sandbox-id
   :image image
...
(defn ^:async sandbox-create!
  ...
  (let [sandbox-id (.randomUUID crypto)
        ...
        context (sandbox-start-context sandbox-id container-name user workdir
                                       expires-at host-dir image)
        metadata (initial-sandbox-metadata sandbox-id image created-at-ms
                                           max-expires-at ttl expires-at)]
    (await (ensure-sandbox-image! runtime config image))
    (try
      (await (prepare-sandbox-state! runtime config sandbox-id host-dir metadata))
      (await (start-sandbox-container! runtime config context))
      (catch :default err
...
(defn- ^:async sandbox-exec!
  ...
  (let [info (await (ensure-live-sandbox! runtime config sandbox-id))
        result (await (docker-command! runtime config
...
(defn ^:async sandbox-create-execute [runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))]
    (maybe-tool-update! on-update "Creating sandbox container…")
    (let [result (await (sandbox-create! runtime config {:ttl-seconds (aget params "ttl_seconds")
...
(defn ^:async sandbox-status-execute [runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        sandbox-id (or (aget params "sandbox_id") (aget params "sandboxId") "")]
    (maybe-tool-update! on-update (str "Inspecting sandbox " sandbox-id "…"))
    (let [result (await (ensure-live-sandbox! runtime config sandbox-id))]
      (if result
        (tool-text-result
...
(defn ^:async sandbox-exec-execute [runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        sandbox-id (or (aget params "sandbox_id") (aget params "sandboxId") "")
    ...
    (let [result (await (sandbox-exec! runtime config sandbox-id command timeout-ms))]
      (tool-text-result
...
(defn ^:async sandbox-read-execute [runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        sandbox-id (or (aget params "sandbox_id") (aget params "sandboxId") "")
...
(defn ^:async sandbox-write-execute [runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        sandbox-id (or (aget params "sandbox_id") (aget params "sandboxId") "")
        path (normalize-sandbox-path (aget params "path"))
...
(defn ^:async sandbox-commit-execute [runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        sandbox-id (or (aget params "sandbox_id") (aget params "sandboxId") "")
...
(defn ^:async sandbox-destroy-execute [runtime config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        sandbox-id (or (aget params "sandbox_id") (aget params "sandboxId") "")]
    (maybe-tool-update! on-update (str "Destroying sandbox " sandbox-id "…"))
    (let [result (await (sandbox-destroy! runtime config sandbox-id))]

...
(defn index-approvals
  "Index approvals by `[document locale revision]`.
...
(defn approved?
  "Whether a *current* approval exists for this document, target locale and
   concrete source revision.
  ...
  [evidence document locale revision]
  (let [entry-key (evidence-key document locale revision)
        receipt (get-in evidence [:receipts entry-key])]
    (boolean
     (and (some? receipt)
          (some (fn [approval]
                  (and (law/approval-matches? approval document locale revision)
...
(defn gate-facts
  ...
  {:translated-revision? (fn [document locale revision]
   ...
   :approved? (fn [document locale revision]
                (approved? evidence document locale revision))})

...
(defn- update-profile
  ...
  (let [user (law/present! state :users user-id)
    ...
    (when (and (some #(contains? payload %) [:display-name :status])
               (not (policy/administrator? ctx)))
      (law/refuse! 403 "local_policy_global_user_profile" "Only a platform administrator may change the global user profile"))

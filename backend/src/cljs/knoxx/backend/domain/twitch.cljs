...
(defn- ^:async get-twitch-config! [runtime]
  (let [credential (await (actor-credentials/get-credential! runtime "twitch"))
        token (actor-credentials/secret-value credential :oauthToken :oauth-token :token)
...
(defn- ^:async connect-twitch! [runtime]
  (let [{:keys [token user]} (await (get-twitch-config! runtime))]
...
(defn- ^:async twitch-send-message! [runtime channel-id text]
  (let [conn (await (connect-twitch! runtime))
        formatted-channel (str "#" (str/replace channel-id #"^#" ""))]
    (.send conn (str "PRIVMSG " formatted-channel " :" text))
    {:sent true :channel channel-id :text text}))

(defn- ^:async twitch-join-channel! [runtime channel-id]
  (let [conn (await (connect-twitch! runtime))
        formatted-channel (str "#" (str/replace channel-id #"^#" ""))]
...
(defn ^:async twitch-send-execute [runtime _config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
    ...
    (let [result (await (twitch-send-message! runtime channel-id text))]
      (tool-text-result (str "Sent Twitch message to " channel-id)
                        result))))

(defn ^:async twitch-read-execute [_runtime _config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        channel-id (or (aget params "channel_id") (aget params "channelId") "")]
    (maybe-tool-update! on-update (str "Reading Twitch chat from " channel-id "…"))
    (let [result (await (twitch-read-messages! channel-id))
...
(defn ^:async twitch-join-execute [runtime _config _tool-call-id params a b c]
  (let [on-update (or (when (fn? a) a) (when (fn? b) b) (when (fn? c) c))
        channel-id (or (aget params "channel_id") (aget params "channelId") "")]
    (maybe-tool-update! on-update (str "Joining Twitch channel " channel-id "…"))
    (let [result (await (twitch-join-channel! runtime channel-id))]

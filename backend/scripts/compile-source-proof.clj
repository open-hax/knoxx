    ...
    (let [config (edn/read-string (slurp "shadow-cljs.edn"))
          build (-> (get-in config [:builds :test-ci])
                    (assoc :build-id :source-authority-proof
                           ...
                           :ns-regexp "^knoxx\\.backend\\.(domain\\.source-review-test|domain\\.source-authoring-test|infra\\.clio-application-store-test|infra\\.clio-subscription-test|source-recovery-test|source-acceptance-test)$"))]

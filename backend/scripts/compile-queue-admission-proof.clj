  ...
  (shadow/with-runtime
    (let [config (edn/read-string (slurp "shadow-cljs.edn"))
          build (-> (get-in config [:builds :test-ci])
                    (assoc :build-id :queue-admission-proof
                           :output-to "target/queue-admission-proof/tests.cjs"
                           :ns-regexp "knoxx\\.backend\\.extern\\.(event-queue|session-reclaim)-admission-test$"))]

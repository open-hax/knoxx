        ...
        build (-> (get-in config [:builds :test-ci])
                  (assoc :build-id (if native? :run-events-native-proof :run-events-proof)
                         ...
                         :ns-regexp (if native? "knoxx\\.backend\\.mongo-(run-events|thread-atomicity|event-chunks)-e2e$"
                                      "knoxx\\.backend\\.((mongo-event-chunks|run-event-provider|run-queries-recovery|openplanner-session-store|mongo-thread-cache|agent-run-persistence)-test|infra\\.stores\\.mongo-(run|session)-store-test|extern\\.(hydration-publication|initial-admission-cleanup|startup-publication|async-spawn-durability|agent-control-persistence|cache-startup|turn-finalization|bootstrap-readiness|run-provider-startup|event-queue-admission|event-queue-retention)-test)$")))]

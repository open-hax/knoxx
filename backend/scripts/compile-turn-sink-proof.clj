  ...
  (let [config (edn/read-string (slurp "shadow-cljs.edn"))
        build (-> (get-in config [:builds :test-ci])
                  (assoc :build-id :turn-sink-proof
                         ...
                         :ns-regexp "knoxx\\.backend\\.(extern\\.(turn-sink-ownership|turn-finalization|initial-admission-cleanup)-test|agent-(turn-timeout|turn-admission|run-persistence)-test)$"))]

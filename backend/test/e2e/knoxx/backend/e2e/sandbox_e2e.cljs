...
(defn- ^:async destroy-quietly!
  ...
  (when sandbox-id
    (try
      ...
      (catch :default err
        (.warn js/console "[sandbox-e2e] destroy failed; a container may be left behind"
               (str sandbox-id) err)))))
      ...
      (let [started (await (harness/start!))
        ...
        (try
          ...
          (when-let [sandbox-id @id*]
            ...
            (testing "exec runs a command inside the container and sees that file"
              (let [ran (await (call! client "sandbox_container_exec"
                ...
                (is (str/includes? (str (:detail ran)) probe-content)
                    (str "exec did not observe the written file — the write and the "
                         "exec may not share a workdir: " (:detail ran)))))
            ...
            (testing "exec is genuinely inside a container, not on this host"
              (let [ran (await (call! client "sandbox_container_exec"
                ...
                (is (= :ok (:status ran))
                    (str "sandbox_container_exec failed: " (:detail ran)))
                (is (not (str/includes? (str (:detail ran)) (.hostname os)))
                    (str "the command reported this host's hostname, so it did not "
                         "run in a container: " (:detail ran)))))

            (testing "commit records the working tree"
              (let [committed (await (call! client "sandbox_container_commit"
                                            {:sandbox_id sandbox-id
                                             :message "e2e probe"}))]
                (is (not= :rpc-error (:status committed))
                    (str "sandbox_container_commit was refused: " (:detail committed)))))

            (testing "destroy removes the sandbox, and a later call knows it is gone"
              ...
              (let [after (await (call! client "sandbox_container_status"
                                        {:sandbox_id sandbox-id}))]
                (is (not= :ok (:status after))
                    (str "status still reported a destroyed sandbox as live: "
                         (:detail after))))))

...
(deftest wire-keeps-ids-and-rejects-authority-fields
  (let [decoded (wire/decode-send "tool-id" #js {:operation_id "caller-retry-id" :target "actor:reader" :content "hello"
                                               :org_id "foreign" :actor_id "impersonated" :mode "inbox_only"} nil)]
    (is (= "caller-retry-id" (:operation-id decoded))) (is (= "inbox-only" (:mode decoded)))
    ...
    (try
      ...
      (with-redefs [authz/current-context! (fn [_ _] @current)]
        ...
        (let [tool (aget (tools/create-tools {} {} context) 0)]
          (await ((aget tool "execute") "mcp" #js {:operation_id "sdk-message" :target "actor:sender" :content "SDK message" :mode "inbox-only"} nil nil nil))
          (await ((aget tool "execute") "mcp" #js {:operation_id "sdk-second" :target "actor:sender" :content "Another SDK message" :mode "inbox-only"} nil nil nil))
          (await ((aget tool "execute") "fresh-transport-id" #js {:operation_id "sdk-message" :target "actor:sender" :content "SDK message" :mode "inbox-only"} nil nil nil)))
        (is (= #{"http-message" "sdk-message" "sdk-second"} (set (map :id (get-in (await (response app "GET" "/api/actors/mailbox" nil)) [:body :entries])))))
        ...
        (is (= 200 (:status (await (response app "POST" "/api/actors/messages"
                                             {:operation_id "changes" :target "actor:sender"
                                              :mode "inbox-only" :content "A readable message named changes"})))))
        (is (= "A readable message named changes"
               (get-in (await (response app "GET" "/api/actors/mailbox/changes" nil)) [:body :entry :content])))
        ...
        (reset! current nil)
        (is (= 401 (:status (await (response app "GET" "/api/actors/mailbox" nil)))))
        (is (= 401 (:status (await (response app "GET" "/api/actors/mailbox/events/stream" nil))))))

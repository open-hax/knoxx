...
(def ^:private context {:org-id "org-a" :user-id "user-a" :membership-id "member-a" :actor-binding "reader"
              :permissions ["agent.chat.use"]})
(def ^:private scope {:org-id "org-a" :actor-id "sender" :admin? false})
(def ^:private message {:mailbox/id "visible" :mailbox/kind "actor-message"
...
(defn- reply-fixture []
  (let [raw (events/EventEmitter.) frames (atom []) ended (atom false)]
...
(test/deftest ^:async stream-is-scoped-and-only-successful-state-changes-invalidate
    ...
    (try
      (let [provider (clio/open! {:directory directory})]
        ...
        (with-redefs [authz/current-context! (fn [_ _]
          ...
          (await (tick!))
          (test/is (= 1 (count @frames)))
          (await (mailbox/create-entry! provider scope message))
          (await (tick!))
          (test/is (= 2 (count @frames)))
          (await (mailbox/create-entry! provider scope message))
          (test/is (= 409 (:status (await (attempt! #(mailbox/create-entry! provider scope
                                                                                      (assoc message :mailbox/content "changed")))))))
          (test/is (= "append_refused"
                 (:code (await (attempt!
      ...
      (catch :default error (test/is false (str "Unexpected mailbox SSE failure: " error)))
      (finally (.emit raw "close") (registry/install! previous) (fs/remove-tree! directory)))))

(test/deftest invalid-post-append-hook-is-rejected-before-creating-files
  (let [directory (disk/temp-directory!) child (str directory "/invalid")]
    (try
      ...
      (test/is (false? (fs/exists? child)))
      (finally (fs/remove-tree! directory)))))

(test/deftest ^:async hijacked-mailbox-stream-does-not-trigger-a-second-fastify-send
    ...
    (try
      ...
      (with-redefs [authz/current-context! (fn [_ _] context)]
        ...
        (let [response (await (.inject app #js {:method "GET" :url "/stream"}))]
          (test/is (= 200 (.-statusCode ^js response)))
          (test/is (= "event: mailbox-changed\ndata: {}\n\n" (.-body ^js response)))
          (test/is (zero? @implicit-sends))))
      (finally

...
(deftest profile-payload-preserves-omitted-fields-and-explicit-removals
  (is (= {:org-id "org" :display-name "Name"}
         (#'admin-routes/actor-update-payload {:orgId "org" :displayName "Name"})))
  (is (= {:org-id "org" :role-slugs [] :tool-policies [] :status "disabled"}
         (#'admin-routes/actor-update-payload {:orgId "org" :roleSlugs [] :toolPolicies [] :status "disabled"})))
  (is (= [:authProvider] (:identity-fields (#'admin-routes/actor-update-payload {:authProvider "forged"})))) )

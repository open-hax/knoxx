...
(deftest live-run-event-sink-forwards-observability-effects
  (let [calls* (atom [])
    ...
    (with-redefs [run-state/append-run-event! (fn [run-id event]
                  ...
                  session-store/mark-session-streaming! (fn ([session-id active?]
                                                              (swap! calls* conj [:mark-streaming session-id active?]))
                                                            ([_db session-id active?]
                                                              (swap! calls* conj [:mark-streaming session-id active?])))

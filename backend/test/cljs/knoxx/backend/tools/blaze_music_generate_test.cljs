                                ...
                                (fn []
                                  (let [raw (join-chunks @chunks)
                                        parsed (try
                                                 ...
                                                 (catch :default err
                                                   {:_parse_error (.-message err)
                                                    :_raw raw}))
                                        json (.stringify js/JSON (clj->js response-payload))]
                                    (swap! requests* conj parsed)
...
(defn- start-mock-proxx!
  "Start a minimal Proxx-like HTTP server that serves Blaze-compatible JSON for
   ...
   Returns a promise of {:server <Server> :base-url <string>}"
  [hex-audio]
  (p/let [{:keys [server base-url]}
          (start-mock-proxx-recording!
           {:id "mock-music-1"
            :object "music.generation"
            :status "success"
            :model "MiniMax-music-2.6-highspeed"
            ;; Keep response-text small; Blaze saving logic should still find the asset
...
(deftest music-generate-song-saves-hex-audio-payload
  (async done
    (testing "music.generate_song writes a valid mp3 buffer when Blaze returns hex audio"
      (let [workspace-root (str "/tmp/knoxx-blaze-mock-" (.randomUUID crypto))
        ...
        (-> (p/let [fixture-bytes (.readFile fs fixture-mp3)
                    hex (.toString fixture-bytes "hex")
                    {:keys [server base-url]} (start-mock-proxx! hex)
                    _ (reset! server* server)
                    runtime #js {}
                    config* (assoc config :proxx-base-url base-url)
                    ...
                    saved-bytes (.readFile fs absolute)
...
(deftest music-generate-song-saves-asset-even-when-status-terminated
  (async done
...
(deftest music-generate-song-sends-lyrics-and-normalizes-section-tags
  (async done
    (testing "music.generate_song sends Blaze-compatible lyrics payload (matching the known-working Python shape)"
      (let [workspace-root (str "/tmp/knoxx-blaze-mock-" (.randomUUID crypto))
            output-path (str "Music/blaze/mock-" (.randomUUID crypto) ".mp3")
            server* (atom nil)
            ...
            config (models/enrich-config
                    {:contracts-dir "contracts"
                     ...
                     :proxx-auth-token "test-token"})]
        (-> (p/let [{:keys [server base-url requests*]}
                    (start-mock-proxx-recording! response)
                    _ (reset! server* server)
                    runtime #js {}
                    config* (assoc config :proxx-base-url base-url)
                    _ (.mkdir fs workspace-root #js {:recursive true})
...
(deftest music-generate-song-appends-vocal-arrangement-when-prompt-not-vocal
  (async done
...
(deftest music-generate-song-infers-wav-from-output-path
  (async done

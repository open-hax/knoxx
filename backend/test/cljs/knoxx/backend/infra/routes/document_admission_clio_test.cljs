...
(test/deftest ^:async source-admission-persists-and-replays-through-the-selected-clio-provider
  (let [directory (disk/temp-directory!)]
    (try
      (let [provider (local/open! (options directory))
        ...
        (test/is (nil? (:error first-result)))
        (test/is (= 1 (get-in first-result [:result :count])))
        (test/is (true? (get-in retry [:result :existing])))
        (test/is (= 1 (get-in retry [:result :index-result :vector-count])))
        (test/is (= source-event (await (client/event-by-id! reopened (:id source-event))))))
      (finally (fs/remove-tree! directory)))))

(test/deftest ^:async failed-indexing-keeps-the-source-fact-and-retry-repairs-its-projection
  (let [directory (disk/temp-directory!)]
    (try
      (let [provider (local/open! (assoc (options directory) :embed!
        ...
        (test/is (= "fixture_model_unavailable" (get-in failed [:error :code])))
        (test/is (= source-event (await (client/event-by-id! provider (:id source-event)))))
        (let [reopened (local/open! (options directory))
              retry (await (attempt! #(admission/persist-openplanner-event! {} reopened source-event)))]
          (test/is (nil? (:error retry)))
          (test/is (true? (get-in retry [:result :existing])))
          (test/is (= [(:id source-event)] (get-in retry [:result :index-result :repaired-event-ids])))
          (test/is (= [[(:id source-event)]]

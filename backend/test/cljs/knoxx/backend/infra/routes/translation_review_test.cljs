...
(defn- receipt
  [& {:keys [org project locale revision translation-revision]
      :or {org "org-1"
           project "knoxx-session"
           locale :es
...
(defn- request
  [& {:keys [locale revision translation-revision document]
...
(deftest ^:async approving-twice-is-the-same-fact-not-two
  (let [deps (await (store-with! [(receipt)]))
    ...
    (testing "only one approval exists"
      ...
      ;; clicked.
      (is (= 1 (count (await (store/approvals! (:evidence-store deps)))))))))
...
(deftest ^:async an-approval-with-no-translation-behind-it-is-refused
  (let [deps (await (store-with! []))
    ...
    (testing "nothing was persisted"
      ...
      ;; evidence.
      (is (empty? (await (store/approvals! (:evidence-store deps))))))))

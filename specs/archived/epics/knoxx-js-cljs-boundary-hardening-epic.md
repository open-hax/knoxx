...
Completed 2026-05-21. The generic extern import allow-list is empty, and `pnpm -C backend boundary:check` reports zero allow-listed non-extern generic extern import files. Final drain also removed generic `extern.js` imports from `infra/routes/app.cljs` and `infra/agent/session.cljs` by routing promise aggregation and extension lifecycle payload construction through named boundary adapters.

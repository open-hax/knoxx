...
---
Triage 2026-05-29: Well-scoped spec epic with clear purpose — formalising the Shibboleth-lite human-in-the-loop labeling workflow for corporate knowledge QA. Spec is detailed with complete label schema (8 dimensions), DSL pseudocode, API contracts, review UI layout, and a thorough gap analysis against existing code. Partial implementations exist (TypeScript/Python schemas, label API, export pipeline, KnowledgeLabeler UI) and gaps are clearly identified (DSL macros for corporate QA not yet built, Ragussy→futuresight-kms handoff path missing, real data fetching absent from UI). No external blockers — all dependencies are internal and either already in the repo or within the team's control. Needs breakdown into sub-tasks (DSL macro work, Ragussy integration, UI queue wiring). Verdict: accepted (P2). --tasks-dir /home/err/devel/orgs/open-hax/openplanner/packages/agents/knoxx/kanban
---
